#include "hybrid_plant_recycle.hpp"
...
HybridPlantRecycleResult run_hybrid_plant_recycle(
    ...
    const dispatch::StorageState& initial_storage_state, const HybridPlantRecycleConfig& cfg) {
  HybridPlantRecycleResult res;
  ...
  res.reactor_chain = run_co2_h2_plant_with_recycle(cfg.co2_feed_kg_s, h2_kg_s, cfg.recycle_cfg);
  ...
  items.push_back(economics::cost_reactor_as_shell_and_tube(
      "methanol synthesis reactor", cfg.recycle_cfg.plant_cfg.reactor_cfg.bed.n_tubes > 1
          ? cfg.recycle_cfg.plant_cfg.reactor_cfg.bed
  ...
  res.annual_opex = economics::compute_opex(
      flows, res.dispatch_avg_electricity_MW, cfg.reactant_prices,
  ...
  const double annual_production_t =
      res.reactor_chain.meoh_production_kg_s * kSecondsPerYear * cfg.reactant_prices.stream_factor / 1000.0;
  ...
  res.ok = true;
  res.message = "ok. Production is the crude methanol leaving the loop, before any separation. " + res.capex.message;
  return res;
}
