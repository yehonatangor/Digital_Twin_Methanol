#include "hybrid_plant_recycle_purified.hpp"
...
HybridPlantRecyclePurifiedResult run_hybrid_plant_recycle_purified(
    ...
    const HybridPlantRecyclePurifiedConfig& cfg) {
  HybridPlantRecyclePurifiedResult res;
  ...
  res.ok = true;
  res.message = "ok. Production is the distillate's own mass flow, so recovery and purity are "
      "accounted for. The distillation column's own capital cost is still NOT in this CAPEX. " +
      res.capex.message;
  return res;
}
